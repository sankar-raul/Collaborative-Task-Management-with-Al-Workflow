...
class ProjectService {
    ...
    static async updateProject(projectId: string, updateData: Partial<IProject>) {
        try {
            const project = await ProjectModel.findByIdAndUpdate(projectId, updateData, { new: true });
            if (!project) {
                throw new Error('Project not found');
            }
            return project;
    ...
    static async updateMemberRole(projectId: string, userId: Types.ObjectId, newRole: Exclude<IUser["role"], "Admin">) {
        try {
            const project = await ProjectModel.findById(projectId);
            if (!project) {
                throw new Error('Project not found');
            }
            const member = project.members.find(member => member.user.equals(userId));
            if (!member) {
                throw new Error('User is not a member of the project');
            }
            member.role = newRole;
